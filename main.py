...
class MenuButtonFilter(Filter):
    async def __call__(self, message: types.Message, state: FSMContext) -> bool:
        ...
        menu_buttons = [
            texts.buttons["new_message"],
            texts.buttons["favorite_messages"],
            texts.buttons["message_history"],
...
class LoadingMessageManager:
    def __init__(self, message: types.Message):
        ...
        self.task = None
        self.current_operation = "🔍 Начинаю конвертацию текста..."
...
@router.message(lambda msg: msg.text == texts.buttons["new_message"])
async def new_recipe_request(message: types.Message, state: FSMContext):
    await message.answer(
        "Пожалуйста, напишите ваше сообщение для конвертации.",
...
async def process_recipe_request(message: types.Message, state: FSMContext):  # Main func
    if message.text == "Отмена":
        await message.answer(
            "Конвертация текста отменена",
            reply_markup=get_main_keyboard()
        )
        await state.clear()
        return

    loading_message = await message.answer("🔍 Начинаю конвертацию текста...")
...
@router.message(StateFilter(PreferenceStates.waiting_for_menu_choice),
                lambda msg: msg.text == "Посмотреть мои предпочтения")
